...
def DecisionTree_train(data_size=None, train_data=None, train_label=None, val_data=None, val_label=None):
    ...
    plt.scatter(pre[:, 0], pre[:, 1], c='#800080', alpha=0.4, marker='o', s=28, cmap='coolwarm', label='DT-pre')
    plt.scatter(val_label[:, 0], val_label[:, 1], c='#ff7f0e', alpha=0.4, marker='^', s=18, cmap='coolwarm', label='True label')
    ...
    print("total time_DT:", time.perf_counter() - start)

    return model,total_time

if __name__ == "__main__":

    model, total_time = DecisionTree_train()
    # validation
    ...
    # validation
    MSE, acc, out_prob_bf, out_prob_predict, op_random, op_bulk, op_persub, op_maxsnr, op_nearest, verify_latency = v.verify_with_outage_prob(size=500000,model=model)
    print("out_prob_predict_DT", out_prob_predict)
    print("acc_DT", acc)
    print("MSE_DT", MSE)